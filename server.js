// ============================================================
// HARAKAPAY SERVER - ADMIN BY EMAIL
// ============================================================
...
// ============================================================
// ADMIN EMAIL CONFIGURATION - WEWEKA EMAIL ZAKO HAPA
// ============================================================
const ADMIN_EMAILS = [
    'gullamanyama0@gmail.com',  // Admin 1
    // 'admin2@example.com',    // Admin 2
    // 'admin3@example.com',    // Admin 3
];
...
try {
    ...
    console.error('⚠️ Check your service account credentials');
}
...
// ============================================================
// ADMIN VERIFICATION - CHECKS EMAIL FROM CONFIG
// ============================================================
async function verifyAdmin(req, res, next) {
    ...
    try {
        ...
        // Check if user is admin by email
        const isAdminUser = await isAdminByEmail(userEmail);

        if (!isAdminUser) {
            return res.status(403).json({
                success: false,
                error: 'Admin access required. Only authorized admins can access this panel.'
            });
        }
        ...
        if (userData.role !== 'admin') {
            ...
            console.log(`✅ User ${userEmail} role updated to admin automatically`);
        }
...
// ============================================================
// GET ADMIN EMAILS (For debugging)
// ============================================================
app.get('/api/admin/emails', verifyAuthToken, verifyAdmin, (req, res) => {
    res.json({
        success: true,
...
app.post('/api/auth/signup', async (req, res) => {
    try {
        ...
        try {
            ...
            console.log(`✅ User saved to Firestore with role: ${userRole}`);
        } catch (firestoreError) {
    ...
    try {
        ...
        try {
            ...
            if (isAdminUser && userData.role !== 'admin') {
                ...
                console.log(`✅ User ${email} role updated to admin on login`);
            }
...
if (!HARAKAPAY_API_KEY) {
    console.warn('⚠️ HARAKAPAY_API_KEY not set. Payment features will not work.');
}
